...
#' Core kernel method for 1D density estimation and regression
#'
#' @param X Vector of input points (nObs)
#' @param x Vector of evaluation points (nEval, if NULL, generated by defineEvalPoints)
#' @param nEval Number of evaluation points if x is NULL
#' @param kernel.type Type of kernel function to use (default: "gauss")
#' @param D Pre-computed distance components (if NULL, computed internally)
#' @param method.h Method for bandwidth selection (if NULL, specified h is used)
#' @param h Bandwidth parameter (if NULL, selected by method.h)
...
#'
#' @return A list containing the estimation results and parameters
kernelMethod1d <- function(X, x=NULL, nEval=100, kernel.type="gauss", D=NULL,
                             method.h=NULL, h=NULL, lambda=NULL,
                             gc=FALSE, chunk_size=if (is.null(x)) nEval else length(x), type.est=NULL, Y=NULL) {
    ...
    if (is.null(x)) { x = defineEvalPoints1d(X, nEval) }
    ...
    kernelFunction = defineKernel1d(kernel.type)
    ...
    for(i in 1:length(chunks)) {
        ...
        if (is.null(D)){ D_chunk = computeDcomponents1d(Z, z_chunk)
        } else { D_chunk = D[,chunk] }

        # Kernel computation
        K = kernelFunction(sweep(D_chunk, 1, h * lambda, "/"))
        if (gc == TRUE){ gc() }

        K_scaled = sweep(K, 1, lambda, "/")

        switch(type.est,
               "density" = {
                   estimator[chunk] = computeTerms1d(D_chunk, Y, h, sdX, K_scaled, type.est)
               },
               {
                   stop(paste("Invalid type.est:", type.est, ". Must be 'density' for 1D."))
               }
        )
        if (gc == TRUE){ gc() }
    }
...
#' Computes terms for different 1D kernel estimation methods
#'
#' @param distances Distance components between input and evaluation points (nObs x nEval_chunk)
#' @param Y Vector of response values (nObs, for regression methods)
#' @param h Bandwidth parameter
#' @param sdX Standard deviation of X
#' @param K_scaled Matrix of scaled kernel weights (nObs x nEval_chunk)
#' @param type.est Type of estimation: "density"
#'
#' @return Vector of density values (nEval_chunk)
computeTerms1d <- function(distances, Y, h, sdX, K_scaled, type.est){
    nObs = dim(distances)[1]

    switch(type.est,
        "density" = {
            return(1/(nObs * h * sdX) * colSums(K_scaled))
        }
    )
}
...
#' Selects a 1D kernel function based on kernel type
#'
#' @param kernel.type Type of kernel function ("epa" or "gauss")
#'
#' @return The corresponding 1D kernel function
defineKernel1d <- function(kernel.type) {
    if (is.null(kernel.type) || kernel.type == "epa"){ return(epaKernel1d) }
    else if (kernel.type == "gauss") { return(gaussKernel1d) }
    else { stop("kernel not recognized") }
}
